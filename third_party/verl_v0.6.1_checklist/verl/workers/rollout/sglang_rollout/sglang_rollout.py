...
class SGLangRollout(BaseRollout):
    def __init__(
        ...
        tool_names = sorted(self._tool_map.keys())
        logger.info(
            "Initialized rollout tools: count=%s preview=%s tool_call_parser_type=%s function_call_parser=%s",
            len(tool_names),
            tool_names[:10],
            self._tool_call_parser_type,
            type(self._function_call_parser).__name__ if self._function_call_parser is not None else None,
        )
    ...
    @staticmethod
    def _normalize_qwen25_assistant_content(content: str, has_tool_calls: bool = False) -> str:
        # Preserve the original assistant content, including <think> blocks. For Qwen2.5/Qwen3,
        # the chat template itself decides which historical reasoning blocks remain visible after
        # later user turns. Stripping reasoning here changes the rollout prompt semantics and can
        # diverge from the template-driven incremental path.
        if content is None:
            return ""
    ...
    def _resolve_interaction_name(self, interaction_kwargs: dict[str, Any]) -> str:
        interaction_name = interaction_kwargs.get("name")
        if interaction_name:
            return interaction_name
        if len(self.interaction_map) == 1:
            return next(iter(self.interaction_map))
        if "checklist" in self.interaction_map:
            return "checklist"
        return "gsm8k"

    @staticmethod
    def _has_active_interaction(interaction_kwargs: dict[str, Any]) -> bool:
        if not interaction_kwargs:
            return False
        meaningful_kwargs = {k: v for k, v in interaction_kwargs.items() if k != "all_messages"}
        return bool(meaningful_kwargs)
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        while current_turns < self.config.multi_turn.max_assistant_turns:
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if finish_reason_type == FinishReasonTypeEnum.LENGTH:
                    normalized_content = self._normalize_qwen25_assistant_content(content)
                    _req.add_assistant_message(
                        self.processing_class,
                        content=normalized_content,
                        content_ids=content_ids if normalized_content == content else None,
                    )
                    break
                else:
                    if self._function_call_parser and self._function_call_parser.has_tool_call(content):
                        ...
                        if len(parsed_tool_calls) > 0:
                            _req.add_assistant_message(
                                self.processing_class,
                                content=self._normalize_qwen25_assistant_content(normed_content, has_tool_calls=True),
                        ...
                        if (
                            self._has_active_interaction(_req.interaction_kwargs)
                ...
                interaction_name = self._resolve_interaction_name(_req.interaction_kwargs)
        ...
        response_loss_tokens = (
            int(_req.response_loss_mask.sum().item()) if _req.response_loss_mask is not None else None
        )
        response_token_len = int(_req.response_ids.shape[-1]) if _req.response_ids is not None else None
        if response_loss_tokens in (None, 0):
            last_role = _req.messages[-1].role if _req.messages else None
            last_has_tool_calls = bool(_req.messages and _req.messages[-1].tool_calls is not None)
            logger.warning(
                "Empty rollout response detected: request_id=%s finish_reason=%s state=%s "
                "assistant_turns=%s user_turns=%s response_token_len=%s response_loss_tokens=%s "
                "last_role=%s last_has_tool_calls=%s metrics_keys=%s",
                _req.request_id,
                finish_reason_type.value if finish_reason_type is not None else None,
                _req.state.value if _req.state is not None else None,
                current_turns,
                user_turns,
                response_token_len,
                response_loss_tokens,
                last_role,
                last_has_tool_calls,
                list(_req.metrics.keys()) if isinstance(_req.metrics, dict) else None,
            )
    ...
    async def _handle_pending_state(self, _req: AsyncRolloutRequest) -> AsyncRolloutRequest:
        if _req.tool_schemas is not None:
            ...
            for tool_schema in _req.tool_schemas:
                ...
                if tool is None:
                    logger.warning(
                        "Skipping tool initialization for unregistered tool schema: request_id=%s tool=%s",
                        _req.request_id,
                        tool_name,
                    )
                    continue
        ...
        if self._has_active_interaction(_req.interaction_kwargs) and self.interaction_map:
            interaction_kwargs = _req.interaction_kwargs
            # Get interaction by name from interaction_kwargs
            interaction_name = self._resolve_interaction_name(interaction_kwargs)
            if interaction_name not in self.interaction_map:
                raise ValueError(
                    f"Interaction '{interaction_name}' not found in interaction_map. Available interactions: "
                    f"{list(self.interaction_map.keys())}"
    ...
    def _create_padding_request(self, original_req: AsyncRolloutRequest) -> AsyncRolloutRequest:
        ...
        logger.warning(
            "Creating padding rollout request: request_id=%s prompt_len=%s max_response_len=%s "
            "original_state=%s last_role=%s",
            original_req.request_id,
            int(original_req.prompt_ids.shape[-1]) if original_req.prompt_ids is not None else None,
            padding_response_length,
            original_req.state.value if original_req.state is not None else None,
            original_req.messages[-1].role if original_req.messages else None,
        )
        return padding_req
...
class ServerAdapter(BaseRollout):
    ...
    async def _init_server_adapter(self):
        ...
        self._engine = AsyncHttpServerAdapter(
            ...
            max_attempts=server_cfg.max_attempts,
            retry_delay=server_cfg.retry_delay,
