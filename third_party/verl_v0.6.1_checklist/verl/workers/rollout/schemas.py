...
_TOKENIZATION_DIFF_DEBUG_PATH = os.getenv(
    "VERL_TOKENIZATION_DIFF_DEBUG_PATH",
    "/mnt/shared-storage-user/zhouyijin/workspace/MyProj/UQ/debug/verl_tokenization_diff_debug.jsonl",
)
_TOKENIZATION_DIFF_DEBUG_ENABLE = os.getenv("VERL_TOKENIZATION_DIFF_DEBUG", "1").lower() not in {"0", "false", "no"}
...
class AsyncRolloutRequest(BaseModel):
    ...
    def add_assistant_message(
        ...
        # Always recompute assistant deltas from the chat template so incremental prompt construction
        # stays aligned with the full prompt rebuilt in finalize(), especially for Qwen2.5/Qwen3
        # templates that can normalize reasoning blocks or tool-call formatting.
        content_ids = None
        ...
        # Trust the chat template delta as-is. Manually appending "<|im_end|>\n" here duplicates
        # the assistant closing tokens for Qwen2.5/Qwen3 templates and can desynchronize
        # incremental prompt construction from the full prompt rebuilt in finalize().
    ...
    @staticmethod
    def _normalize_prompt_for_reasoning_aware_compare(prompt: str) -> str:
        prompt = _THINK_BLOCK_RE.sub("", prompt)
        prompt = re.sub(r"\n{3,}", "\n\n", prompt)
        return prompt.strip()

    def _prompts_match_after_reasoning_normalization(
        self,
        processing_class: PreTrainedTokenizer | PreTrainedTokenizerFast | ProcessorMixin,
        full_prompt_ids: torch.Tensor,
        current_prompt_ids: torch.Tensor,
    ) -> bool:
        full_prompt = processing_class.decode(full_prompt_ids.squeeze(0), skip_special_tokens=False)
        current_prompt = processing_class.decode(current_prompt_ids.squeeze(0), skip_special_tokens=False)
        return self._normalize_prompt_for_reasoning_aware_compare(full_prompt) == self._normalize_prompt_for_reasoning_aware_compare(current_prompt)

    def _dump_tokenization_diff_debug(
        self,
        processing_class: PreTrainedTokenizer | PreTrainedTokenizerFast | ProcessorMixin,
        full_prompt_ids: torch.Tensor,
        current_prompt_ids: torch.Tensor,
        ...
        if not _TOKENIZATION_DIFF_DEBUG_ENABLE:
            return
        try:
            full_prompt = processing_class.decode(full_prompt_ids.squeeze(0), skip_special_tokens=False)
            current_prompt = processing_class.decode(current_prompt_ids.squeeze(0), skip_special_tokens=False)
            ...
            for msg in reversed(self.messages):
                if msg.role == "assistant":
                    last_assistant_message = msg.model_dump()
                    break
            payload = {
                ...
                "pid": os.getpid(),
                "tokenization_sanity_check_mode": self.tokenization_sanity_check_mode.value,
                "diffs": diffs,
                "messages": [msg.model_dump() for msg in self.messages],
                "last_assistant_message": last_assistant_message,
                "full_prompt": full_prompt,
                "current_prompt": current_prompt,
                "full_prompt_reasoning_normalized": self._normalize_prompt_for_reasoning_aware_compare(full_prompt),
                "current_prompt_reasoning_normalized": self._normalize_prompt_for_reasoning_aware_compare(current_prompt),
            }
            debug_dir = os.path.dirname(_TOKENIZATION_DIFF_DEBUG_PATH)
            if debug_dir:
                os.makedirs(debug_dir, exist_ok=True)
            with open(_TOKENIZATION_DIFF_DEBUG_PATH, "a", encoding="utf-8") as f:
                f.write(json.dumps(payload, ensure_ascii=False) + "\n")
        except Exception as exc:
            logger.warning("Failed to dump tokenization diff debug for request %s: %s", self.request_id, exc)
    ...
    def finalize(
        ...
        if self.tokenization_sanity_check_mode != TokenizationSanityCheckModeEnum.DISABLE:
            ...
            if diffs := self._get_prompt_diffs(
                ...
                if self._prompts_match_after_reasoning_normalization(
                    processing_class, full_prompt_ids, self.input_ids
                ):
                    logger.info(
                        "Ignoring tokenization diff for request %s because prompts match after reasoning normalization.",
                        self.request_id,
                    )
                    diffs = []
            ...
            if diffs:
                ...
                if log_warning:
                    self._dump_tokenization_diff_debug(processing_class, full_prompt_ids, self.input_ids, diffs)
