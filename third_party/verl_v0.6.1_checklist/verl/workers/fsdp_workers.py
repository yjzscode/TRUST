...
def _require_accelerator_backend() -> str:
    current_device_name = get_device_name()
    if current_device_name == "cpu":
        raise RuntimeError(
            "CM2 GRPO FSDP training requires CUDA/NPU, but this worker sees only CPU. "
            f"CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES')!r}, "
            f"torch.cuda.is_available()={torch.cuda.is_available()}, "
            f"torch.cuda.device_count()={torch.cuda.device_count()}. "
            "Run this job in a GPU environment or fix GPU visibility before launching."
        )
    return f"cpu:gloo,{current_device_name}:{get_nccl_backend()}"
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        if not torch.distributed.is_initialized():
            ...
            torch.distributed.init_process_group(
                backend=_require_accelerator_backend(),
                rank=rank,
