...
def compute_rs_metrics(
    ...
    if not response_mask.any():
        return {
            "rollout_rs_empty_response_mask": 1.0,
            "rollout_rs_mean": float("nan"),
            "rollout_rs_ratio_fraction_high": float("nan"),
            "rollout_rs_ratio_fraction_low": float("nan"),
            "rollout_rs_max": float("nan"),
            "rollout_rs_min": float("nan"),
            "rollout_rs_std": float("nan"),
            "rollout_rs_eff_sample_size": float("nan"),
            "rollout_rs_seq_mean": float("nan"),
            "rollout_rs_seq_std": float("nan"),
            "rollout_rs_seq_max": float("nan"),
            "rollout_rs_seq_min": float("nan"),
            "rollout_rs_seq_max_deviation": float("nan"),
            "rollout_rs_seq_fraction_high": float("nan"),
            "rollout_rs_seq_fraction_low": float("nan"),
        }
...
def compute_is_metrics(
    ...
    if not response_mask.any():
        return {
            "rollout_is_empty_response_mask": 1.0,
            "rollout_is_mean": float("nan"),
            "rollout_is_ratio_fraction_high": float("nan"),
            "rollout_is_ratio_fraction_low": float("nan"),
            "rollout_is_max": float("nan"),
            "rollout_is_min": float("nan"),
            "rollout_is_std": float("nan"),
            "rollout_is_eff_sample_size": float("nan"),
            "rollout_is_seq_mean": float("nan"),
            "rollout_is_seq_std": float("nan"),
            "rollout_is_seq_max": float("nan"),
            "rollout_is_seq_min": float("nan"),
            "rollout_is_seq_max_deviation": float("nan"),
            "rollout_is_seq_fraction_high": float("nan"),
            "rollout_is_seq_fraction_low": float("nan"),
        }
...
def compute_rollout_correction_and_rejection_mask(
    ...
    if not response_mask.any():
        metrics: dict[str, float] = {
            "rollout_corr/empty_response_mask": 1.0,
            "rollout_corr/valid_response_tokens": 0.0,
            "rollout_corr/rollout_is_veto_fraction": 0.0,
            "rollout_corr/rollout_is_catastrophic_token_fraction": 0.0,
        }
        return None, response_mask.clone(), metrics
...
def compute_offpolicy_metrics(
    ...
    """
    # Some multi-turn rollouts intentionally inject padding requests with an all-zero
    # response mask to preserve batch shape after aborted/empty generations.
    # These samples should be ignored by loss/diagnostics instead of crashing PPO.
    if not response_mask.any():
        metrics = {
            "empty_response_mask": 1.0,
            "valid_response_tokens": 0.0,
            "training_ppl": float("nan"),
            "training_log_ppl": float("nan"),
        }
            ...
            metrics.update(
                {
                    "kl": float("nan"),
                    "k3_kl": float("nan"),
                    "rollout_ppl": float("nan"),
                    "rollout_log_ppl": float("nan"),
                    "log_ppl_diff": float("nan"),
                    "ppl_ratio": float("nan"),
                    "chi2_token": float("nan"),
                    "chi2_seq": float("nan"),
                }
            )
        return metrics
    ...
    metrics = {}
    metrics["empty_response_mask"] = 0.0
    metrics["valid_response_tokens"] = response_mask.sum().detach().item()
