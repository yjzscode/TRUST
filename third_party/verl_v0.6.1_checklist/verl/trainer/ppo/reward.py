...
_ASYNC_REWARD_FN = None
logger = logging.getLogger(__name__)


def _maybe_import_external_reward_manager(reward_manager_name: str) -> None:
...
def load_reward_manager(
    ...
    _maybe_import_external_reward_manager(reward_manager_name)
...
@ray.remote(num_cpus=1, num_gpus=float(os.getenv("VERL_ASYNC_REWARD_NUM_GPUS", "0")))
def compute_reward_async(data: DataProto, config=None, tokenizer=None, reward_fn=None):
    """
    Load the reward manager and compute the reward for a batch of data.
    This is meant to be run in a separate Ray worker.
    """
    global _ASYNC_REWARD_FN
    if reward_fn is None and _ASYNC_REWARD_FN is not None:
        reward_fn = _ASYNC_REWARD_FN
...
class AsyncRewardWorker:
    def __init__(self, config, model_path: str, cuda_visible_devices: str | None = None):
        self.config = config
        if cuda_visible_devices is not None:
            os.environ["CUDA_VISIBLE_DEVICES"] = str(cuda_visible_devices)
            logger.warning("AsyncRewardWorker using shared CUDA_VISIBLE_DEVICES=%s", cuda_visible_devices)
        self.tokenizer = hf_tokenizer(model_path, trust_remote_code=config.data.get("trust_remote_code", False))
        self.reward_fn = load_reward_manager(
            config, self.tokenizer, num_examine=0, **config.reward_model.get("reward_kwargs", {})
        )
