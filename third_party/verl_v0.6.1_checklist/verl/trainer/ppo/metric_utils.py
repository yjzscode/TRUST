...
def _is_valid_validation_scalar(value: Any) -> bool:
    if value is None:
        return False
...
def _safe_tensor_stats(values: torch.Tensor) -> dict[str, float]:
    if values.numel() == 0:
        return {"mean": float("nan"), "max": float("nan"), "min": float("nan")}
    return {
        "mean": torch.mean(values).detach().item(),
        "max": torch.max(values).detach().item(),
        "min": torch.min(values).detach().item(),
    }
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    score_stats = _safe_tensor_stats(non_aborted_sequence_score)

    reward_stats = _safe_tensor_stats(non_aborted_sequence_reward)

    valid_adv = torch.masked_select(advantages, response_mask)
    valid_returns = torch.masked_select(returns, response_mask)
    adv_stats = _safe_tensor_stats(valid_adv)
    return_stats = _safe_tensor_stats(valid_returns)

    if use_critic:
        values = batch.batch["values"]
        valid_values = torch.masked_select(values, response_mask)
        value_stats = _safe_tensor_stats(valid_values)
    ...
    metrics = {
        # score
        "critic/score/mean": score_stats["mean"],
        "critic/score/max": score_stats["max"],
        "critic/score/min": score_stats["min"],
        # reward
        "critic/rewards/mean": reward_stats["mean"],
        "critic/rewards/max": reward_stats["max"],
        "critic/rewards/min": reward_stats["min"],
        # adv
        "critic/advantages/mean": adv_stats["mean"],
        "critic/advantages/max": adv_stats["max"],
        "critic/advantages/min": adv_stats["min"],
        # returns
        "critic/returns/mean": return_stats["mean"],
        "critic/returns/max": return_stats["max"],
        "critic/returns/min": return_stats["min"],
        **(
            {
                # values
                "critic/values/mean": value_stats["mean"],
                "critic/values/max": value_stats["max"],
                "critic/values/min": value_stats["min"],
                # vf explained var
        ...
        "response/valid_tokens": valid_response_tokens,
        # prompt length
        ...
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                ...
                if isinstance(var_vals[0], str):
                    continue
                valid_var_vals = [val for val in var_vals if _is_valid_validation_scalar(val)]
                if not valid_var_vals:
                    continue
                    ...
                    for n in ns:
                        ...
                        if var2vals.get("pred", None) is not None:
                            vote_data = [
                                ...
                                if _is_valid_validation_scalar(val)
                            ]
                            if len(vote_data) >= n:
    ...
    for data_source, var2metric2uid_vals in data_src2var2metric2uid_vals.items():
        for var_name, metric2uid_vals in var2metric2uid_vals.items():
            for metric_name, uid_vals in metric2uid_vals.items():
                valid_uid_vals = [val for val in uid_vals if _is_valid_validation_scalar(val)]
                if not valid_uid_vals:
                    continue
                data_src2var2metric2val[data_source][var_name][metric_name] = np.mean(valid_uid_vals)
