...
def _resolve_reward_loop_manager_name(config: DictConfig) -> str:
    name = str(config.reward_model.get("reward_manager", "naive") or "naive").strip()
    # Async agent-loop reward workers consume single-sample reward via reward_loop managers.
    # Existing custom reward managers like checklist/mixed are regular PPO reward managers,
    # not reward_loop manager names. Route them through the generic single-sample loop.
    if name in {"checklist", "mixed"}:
        return "naive"
    return name
...
class RewardManagerWorker:
    ...
    def _init_reward_fn(self):
        ...
        reward_loop_manager_name = _resolve_reward_loop_manager_name(self.config)
