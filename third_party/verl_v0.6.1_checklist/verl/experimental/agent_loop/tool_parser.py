...
class ToolParser(ABC):
    ...
    async def truncate_to_tool_call_response_ids(self, responses_ids: list[int]) -> list[int] | None:
        """Trim trailing garbage after the final tool-call block if possible."""
        return None
...
class HermesToolParser(ToolParser):
    ...
    async def truncate_to_tool_call_response_ids(self, responses_ids: list[int]) -> list[int] | None:
        ...
        if not matches:
            return None
        trimmed_text = text[: matches[-1].end()]
        trimmed_ids = await loop.run_in_executor(
            None, lambda: self.tokenizer.encode(trimmed_text, add_special_tokens=False)
        )
        return trimmed_ids if len(trimmed_ids) < len(responses_ids) else None


@ToolParser.register("qwen25")
class Qwen25ToolParser(ToolParser):
    """Parser for Qwen2.5/Qwen3 tool calls rendered as <tool_call> JSON blocks."""
    ...
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:
        ...
        text = text.replace(getattr(self.tokenizer, "pad_token", "") or "", "")
        ...
        for match in matches:
            try:
                ...
                if not isinstance(function_call, dict):
                    raise ValueError("tool_call payload must decode to dict or non-empty list[dict]")
                name = function_call["name"]
                arguments = function_call.get("arguments", {})
                function_calls.append(FunctionCall(name=name, arguments=json.dumps(arguments, ensure_ascii=False)))
            except Exception as e:
                logger.error(f"Failed to decode qwen25 tool call: {e}")
